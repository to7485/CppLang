...
- 그러므로 이 객체들은 프로그래머가 명시적으로 선언하지 않고 사용할 수 있다.
  	...
  	- cout 객체(표준 출력 스트림 객체) : 표준출력장치인 모니터와 연결되어 있다.
...
## 파일입출력
- 대부분의 프로그램은 파일을 취급한다.
- 디스크에 저장되어 있는 파일은 운영체제마다 관리하는 방법이 다르지만 프로그래머 입장에서는 파일을 프로그램과 연결시키는 방법, 파일의 내용을 읽는 방법, 파일을 생성하는 방법 그리고 파일에 기록하는 방법을 알아야 한다.
- C++는 프로그래머가 파일을 다루기 위한 기능을 제공하므로 프로그래머는 파일에 관한 자세한 사항(예를 들면 파일의 위치, 크기, 생성시기 등)을 알고 있지 않더라도 프로그램을 작성할 수 있다.
- C++ 언어는 iostream에서 파생된 파일 입력을 위한 ifstream 클래스와 파일 출력을 위한 ofstream 클래스를 제공하고 있다.
...
- 표준 입출력 스트림에 대한 객체들(cin, cout, cerr, clog)은 어느 프로그램에서나 항상 사용하므로 <iostream>을 포함시킬 때 이미 생성되어 있고 열려 있다.
- 하지만 파일을 사용하려고 할 때에는 프로그래머가 명시적으로 객체를 생성하고 열어야 한다.
```c
ofstream 객체이름;
객체이름.open(파일이름);

//위의 두 동작을 한번에 수행할 수 있다.
ofstream 객체이름(파일이름);
```
- 파일 읽기 과정도 위와 유사하다.
- 먼저 읽을 파일에 대한 스트림을 생성하고 파일을 열어야 하는데, 이 경우 클래스 ifstream을 사용한다.
...
```
- istream의 멤버함수 get()은 파일의 끝 기호(end of file, EOF)를 만나면 false를 반환한다.
- 키보드에서 EOF를 입력하는 방법은 Windows 운영체제하의 PC에서는 ctrl + z를 입력한다.
- 문자 입력이 끝난 후 EOF 기호를 입력하면 그때까지의 내용이 파일에 기록된다.

## 파일모드
- 파일을 열 때에는 어떤 방식으로 파일을 사용할 것인가를 지정할 수 있다.
- 파일 모드는 멤버함수 open()을 호출할 때 파일 이름과 함께 지정할 수 있다.
- ios 클래스에 정의된 파일 모드 상수는 다음과 같다.
	- ios::app : 파일을 쓰기 모드로 여는 경우에 사용되며, 현재 있는 파일의 끝에 새로 추가하는 경우에 사용된다.
 	- ios::ate : 파일 포인터를 파일 제일 마지막으로 이동한다. 임의 액세스(random access) 방법으로 파일을 액세스하는 경우에 사용된다.
  	- ios::binary : 이진 파일 모드로 파일을 사용하고자 할 때 사용된다.
